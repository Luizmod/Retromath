...
sound3 = pygame.mixer.Sound('music.ogg')
dificultad = 10
...
def botones(screen, boton, palabra):
    if boton.collidepoint(pygame.mouse.get_pos()):
        ...
        screen.blit(texto,(int(boton.x+(boton.width-texto.get_width())/2),
                           int(boton.y+(boton.height-texto.get_height())/2)))
    else:
...
def main_menu():
    ...
    while True:
        ...
        for i in range(0,3):
            ...
            rect = pygame.Rect(int(posX), int(posY), width, height)
...
def game():
    ...
    while running:
        ...
        if numero == 1:
            btn = pygame.Rect(int(posX), int(posY), width, height)
